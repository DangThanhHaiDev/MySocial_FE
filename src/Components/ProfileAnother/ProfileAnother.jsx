...
const ProfilePage = () => {
  ...
  const handleAccept = async () => {
    try {
      const response = await axiosInstance.post(
        `/api/friends/accept/profile/${userId}`
      );
  ...
  const getRelationship = async () => {
    try {
      const response = await axiosInstance.get(
        `/api/friends/relationship/${userId}`
      );
      const { data } = response;
  ...
  const handleUnfriend = async () => {
    if (!window.confirm("Bạn chắc chắn muốn hủy kết bạn với người này?"))
      return;
    try {
              ...
              <div className="grid grid-cols-3 gap-2 ">
                {friend &&
                  friend.map((i) => (
                    <div
                      key={i}
                      className="aspect-square shadow-md p-2 space-y-2 cursor-pointer"
